...
import { fetchStaff, deleteStaff, StaffData, UserData } from "@/actions/staff";
...
const StaffListPage = () => {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      await fetchStaffList(); // Refresh the staff list
    ...
    try {
      ...
      if (response.success && response.data) {
        const staffData = Array.isArray(response.data)
          ? response.data
          : response.data || [];
        const totalCount = response.count || staffData.length;
      ...
      toast.error(error.message || "An error occurred while fetching staff");
    } finally {
      setLoading(false);
    }
  }, [
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchTerm, departmentFilter, statusFilter, roleFilter]);

  // Prepare data for the table
  const tableData = staff.map((staffMember) => {
    const user =
      typeof staffMember.user === "object"
        ? staffMember.user
        : ({} as UserData);
    return {
      id: staffMember.id,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {totalItems} {totalItems === 1 ? "staff member" : "staff members"}{" "}
            found
          </p>
        </div>
